...
class HGT(nn.Module):
    ...
    def __init__(
            self, in_dims, hidden_dim, out_dim, num_heads, ntypes, etypes, num_layers, dropout=0.1, use_norm=True):
        """HGT模型
    ...
    def forward(self, g):
        ...
        """
        with g.local_scope():
            hs = {ntype: F.gelu(self.adapt_fcs[ntype](g.nodes[ntype].data['x'])) for ntype in g.ntypes}
