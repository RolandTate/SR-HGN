...
from model_y.Classifier import Classifier
...
def load_params():
    ...
    parser.add_argument('--prefix', type=str, default='CR_HGN')
...
def train(model, G, labels, target, optimizer, scheduler, train_idx, clip=1.0):
    ...
    nodes_representation = model[0](G)
...
def main(params):
    ...
    model = nn.Sequential(cr_hgn, classifier).to(device)
    ...
    logger.info('Training R_HGT with #param: {:d}'.format(get_n_params(model)))
