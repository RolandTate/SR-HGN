...
class RelationGraphConv(nn.Module):
    ...
    def __init__(self, in_feats: tuple, out_feats: int, num_heads: int, dropout: float = 0.0, negative_slope: float = 0.2):
        ...
        self.relation_node_transformation_weights = nn.Parameter(torch.Tensor(num_heads, out_feats))
    ...
    def forward(self, graph: dgl.DGLHeteroGraph, feat: tuple, dst_node_transformation_weight: nn.Parameter,
                src_node_transformation_weight: nn.Parameter, node_relation_transformation_weight: nn.Parameter,
                relation_embedding: torch.Tensor, relation_transformation_weight: nn.Parameter):
        r"""
        ...
        node_relation_transformation_weight: Parameter (input_dst_dim, n_heads * hidden_dim)
        ...
        feat_src = torch.matmul(feat_src, src_node_transformation_weight)
        feat_src = torch.matmul(feat_src, node_relation_transformation_weight).view(-1, self._num_heads, self._out_feats)
...
class RelationAttentionConv(nn.Module):
    ...
    def __init__(self, hidden_dim: int, num_heads: int, dropout: float = 0.0, negative_slope: float = 0.2):
        """
        Relation graph convolution layer
        Parameters
        ----------
    ...
    def forward(self, graph: dgl.DGLHeteroGraph, feat: tuple, src_node_transformation_layer: nn.Linear,
                dst_node_transformation_layer: nn.Linear, relation_src_node_transformation_layer: nn.Linear,
                relation_embedding: torch.Tensor, relation_transformation_weight: nn.Parameter):
        r"""
        ...
        """
        graph = graph.local_var()
        # Tensor, (N_src, input_src_dim), Tensor, (N_dst, input_dst_dim)
        # Tensor, (N_src, input_src_dim)
        feat_src = self.dropout(feat[0])
        # Tensor, (N_dst, input_dst_dim)
        feat_dst = self.dropout(feat[1])
        # Tensor, (N_src, n_heads * hidden_dim) -> (N_src, n_heads * hidden_dim) -> (N_src, n_heads, hidden_dim)
        # feat_src = src_node_transformation_layer(feat_src)
        feat_src = relation_src_node_transformation_layer(feat_src).view(-1, self._num_heads, self._hidden_dim)
        # Tensor, (N_dst, n_heads * hidden_dim) -> (N_dst, n_heads * hidden_dim) -> (N_dst, n_heads, hidden_dim)
        feat_dst = dst_node_transformation_layer(feat_dst)
        dst_after_transformation = feat_dst
        feat_dst = feat_dst.view(-1, self._num_heads, self._hidden_dim)
        # Tensor, (n_heads, 2 * hidden_dim) -> (1, input_dst_dim) * (input_dst_dim, n_heads * hidden_dim)
        relation_attention_weight = torch.matmul(relation_embedding.unsqueeze(dim=0), relation_transformation_weight).view(self._num_heads, 2 * self._hidden_dim)

        # first decompose the weight vector into [a_l || a_r], then
        # a^T [Wh_i || Wh_j] = a_l Wh_i + a_r Wh_j, This implementation is much efficient
        # Tensor, (N_dst, n_heads, 1),   (N_dst, n_heads, hidden_dim) * (n_heads, hidden_dim)
        e_dst = (feat_dst * relation_attention_weight[:, :self._hidden_dim]).sum(dim=-1, keepdim=True)
        # Tensor, (N_src, n_heads, 1),   (N_src, n_heads, hidden_dim) * (n_heads, hidden_dim)
        e_src = (feat_src * relation_attention_weight[:, self._hidden_dim:]).sum(dim=-1, keepdim=True)
        # (N_src, n_heads, hidden_dim), (N_src, n_heads, 1)
        graph.srcdata.update({'ft': feat_src, 'e_src': e_src})
        # (N_dst, n_heads, 1)
        graph.dstdata.update({'e_dst': e_dst})
        ...
        graph.update_all(fn.u_mul_e('ft', 'a', 'msg'), fn.sum('msg', 'feat'))
         # (N_dst, n_heads * hidden_dim), reshape (N_dst, n_heads, hidden_dim)
        dst_features = graph.dstdata.pop('feat').reshape(-1, self._num_heads * self._hidden_dim)

        dst_features = self.relu(dst_features)

        return dst_features, dst_after_transformation
